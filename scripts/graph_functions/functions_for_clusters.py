    ...
    for usuario in lista_de_usuarios:
        ...
        if len(lista_de_cluster) == 0:
            ...
            for cluster in range(0,len(lista_de_cluster)):
                if (usuario == lista_de_cluster[cluster][-1] and longitud_minitabla > 1): # preparamos los cluster para alvergar los nuevos
...
def creacion_grafico(lista_usuarios_sin_repetir, lista_de_cluster_2):
    ...
    for i in range(0,len(df_merged2)):
        for j in range (2,11):
            aux = j-1
            df_merged2['Q'+str(j)+'_m'][i] = df_merged2['Q'+str(j)+'_m'][i] + df_merged2['Q'+str(aux)+'_m'][i]
