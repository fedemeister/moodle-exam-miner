...
# log_json['questions']
columnas = ['pregunta', 'porcentaje_acertadas', 'calificaciones', 'horas', 'respuestas', 'count_correctas',
            'count_incorrectas']
    ...
    if len(json_df) > 0:  # hay preguntas que no salieron nunca
        ...
        for i in range(0, json_df.shape[0]):
            if i == 0:
                ...
                json_df.porcentaje_acertadas[i] = (
                        json_df.count_correctas[i] / (json_df.count_correctas[i] + json_df.count_incorrectas[i]))
                json_df.porcentaje_errores[i] = (
                        json_df.count_incorrectas[i] / (json_df.count_correctas[i] + json_df.count_incorrectas[i]))
            else:
                nota = json_df.calificaciones[i]
                if nota == 1.0:
        ...
        json_df = json_df.rename(columns={"porcentaje_acertadas": "porcentaje_aciertos_acumulado",
                                          "porcentaje_errores": "porcentaje_errores_acumulado"})
        plot = sns.lineplot(data=json_df[['porcentaje_aciertos_acumulado', 'porcentaje_errores_acumulado']],
                            markers=True, linewidth=2.0)
