...
def anonymizer(reverse_log:bool=False):
    ...
    for entry in json_exam_califications:
        if ((entry[1] + ' ' + entry[0]) in name_user_dictionary):
            ...
            entry[2] = entry[1]
            entry[3] = entry[1]
    with codecs.open('files/tool_output/01_anonymized_input/exam_answers_utf8_anonymized.json', 'w', encoding='utf-8') as f:
        json.dump(json_exam_answers, f, ensure_ascii=False, indent=2)
        
    with codecs.open('files/tool_output/01_anonymized_input/exam_califications_utf8_anonymized.json', 'w', encoding='utf-8') as f:
        json.dump(json_exam_califications, f, ensure_ascii=False, indent=2)
        
    with codecs.open('files/tool_output/01_anonymized_input/exam_logs_utf8_anonymized.json', 'w', encoding='utf-8') as f:
