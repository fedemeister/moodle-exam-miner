...
def execution():
    ...
    df_todas_preguntas['Answer'] = df_todas_preguntas['Answer'].replace(['l.idGROUP'],'l.id GROUP', regex=True)
    df_todas_preguntas['Answer'] = df_todas_preguntas['Answer'].replace([u'A1.nacionalidadHAVING COUNT'],u'A1.nacionalidad HAVING COUNT', regex=True)
    df_todas_preguntas['Answer'] = df_todas_preguntas['Answer'].replace([u'AND A1.nacionalidad'],u' AND A1.nacionalidad', regex=True)
    df_todas_preguntas['Answer'] = df_todas_preguntas['Answer'].replace([u'NOT IN\( SELECT socio_id'],u'NOT IN (SELECT socio_id', regex=True)
    df_todas_preguntas['Answer'] = df_todas_preguntas['Answer'].replace([u'WHERE socio_id IN\( SELECT'],u'WHERE socio_id IN (SELECT', regex=True)
    df_todas_preguntas['Answer'] = df_todas_preguntas['Answer'].replace([u'socio_id =\(SELECT'],u'socio_id = (SELECT', regex=True)
    df_todas_preguntas['Answer'] = df_todas_preguntas['Answer'].replace([u'libros L\)GROUP BY'],u'libros L) GROUP BY', regex=True)
    df_todas_preguntas['Answer'] = df_todas_preguntas['Answer'].replace([u'A.apellido1HAVING'],u'A.apellido1 HAVING', regex=True)
    df_todas_preguntas['Answer'] = df_todas_preguntas['Answer'].replace([u'WHERE id <>\(SELECT socio_id'],u'WHERE id <> (SELECT socio_id', regex=True)
    df_todas_preguntas['Answer'] = df_todas_preguntas['Answer'].replace([u'WHERE id <>\(SELECT socio_id'],u'WHERE id <> (SELECT socio_id', regex=True)
    df_todas_preguntas['Answer'] = df_todas_preguntas['Answer'].replace([u's.idLEFT OUTER JOIN'],u's.id LEFT OUTER JOIN', regex=True)
    df_todas_preguntas['Answer'] = df_todas_preguntas['Answer'].replace([u's.apellido1HAVING'],u's.apellido1 HAVING', regex=True)
    df_todas_preguntas['Answer'] = df_todas_preguntas['Answer'].replace([u'SELECTnacionalidad'],u'SELECT nacionalidad', regex=True)
